...
export function Dashboard() {
  ...
  const getProjectTasks = (projectId: string, currentVersionId?: string) => {
    return tasks.filter((task) => {
      if (task.projectId !== projectId) return false;
      if (currentVersionId) return task.versionId === currentVersionId;
      return true;
    });
  };

  const getProjectProgress = (projectId: string, currentVersionId?: string) => {
    const projectTasks = getProjectTasks(projectId, currentVersionId);
    if (projectTasks.length === 0) return 0;
    const completed = projectTasks.filter((t) => t.status === 'done').length;
    return Math.round((completed / projectTasks.length) * 100);
  };

  const getNextTask = (projectId: string, currentVersionId?: string) => {
    // 获取该项目的所有未完成任务
    const projectTasks = getProjectTasks(projectId, currentVersionId)
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        {isDesktop ? (
          <Button onClick={() => navigateToPath('/projects', navigate)}>{t.createProject}</Button>
        ) : (
          <Link to="/projects">
            ...
            <div className="space-y-4 py-2">
              {activeProjects.length === 0 ? (
                ...
                activeProjects.slice(0, 5).map((project) => {
                  const progress = getProjectProgress(project.id, project.currentVersionId);
                  const taskCount = getProjectTasks(project.id, project.currentVersionId).length;
                  return (
        ...
        {activeProjects.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {activeProjects.slice(0, 6).map((project, index) => {
              const progress = getProjectProgress(project.id, project.currentVersionId);
              const taskCount = getProjectTasks(project.id, project.currentVersionId).length;
              const nextTask = getNextTask(project.id, project.currentVersionId);
